...
void run_kernel()
{
    ...
    #ifdef ARCH_WORMHOLE
    _llk_pack_hw_configure_<false, is_fp32_dest_acc_en>(DATA_FORMAT, DATA_FORMAT, (uint32_t)16*16*4);
    #else
    _llk_pack_hw_configure_<false, is_fp32_dest_acc_en, false>(DATA_FORMAT, DATA_FORMAT, (uint32_t)16*16*4);
    #endif
